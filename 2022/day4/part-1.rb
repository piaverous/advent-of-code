...
$lines.each do |line|
    ...
    if (first[0] <= second[0]) && (first[1] >= second[1])
        # If first elf sections include second elf sections
        score += 1
    elsif (first[0] >= second[0]) && (first[1] <= second[1])
        # If second elf sections include first elf sections
        score += 1
    end
end
puts score
